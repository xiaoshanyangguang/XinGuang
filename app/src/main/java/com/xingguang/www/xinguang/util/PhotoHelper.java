...
public class PhotoHelper {
    ...
    public static void fromCamera(String planContentPicId, Object obj) {
        fromCamera(planContentPicId, obj, false);
    }

    public static void fromCamera(String planContentPicId, Object obj, boolean needCrop) {
        if (!checkTargetValided(obj))
            return;
        checkAndRequestCameraPermission(planContentPicId, obj, needCrop, false);
    }
    ...
    private static void checkAndRequestCameraPermission(final String planContentPicId, final Object object, final
    boolean
            needCrop, final boolean
                                                                album) {
        ...
        OnPermissionGrantListener onPermissionGrantListener = new OnPermissionGrantListener() {
            ...
            public void onPermissionGranted(PermissionHelper.Permission... grantedPermissions) {
                if (album) {
                    ...
                    fromCameraInternal(planContentPicId, object, needCrop);
                }
            }
        ...
        if (object instanceof Activity) {
            permissionHelper = object instanceof IPermission ? ((IPermission) object).getPermissionHelper() : new
                    PermissionHelper((Activity) object);
        } else if (object instanceof Fragment) {
            permissionHelper = object instanceof IPermission ? ((IPermission) object).getPermissionHelper() : new
                    PermissionHelper(((Fragment) object).getActivity());
        }
        if (permissionHelper != null) {
            if (album) {
                permissionHelper.requestPermission(onPermissionGrantListener, PermissionHelper.Permission
                        .WRITE_EXTERNAL_STORAGE, PermissionHelper.Permission.READ_EXTERNAL_STORAGE);
            } else {
                permissionHelper.requestPermission(onPermissionGrantListener, PermissionHelper.Permission.CAMERA,
    ...
    private static void fromAlbumInternal(Object obj, boolean needCrop) {
        ...
        if (obj instanceof Fragment) {
            ((Fragment) obj).startActivityForResult(albumIntent, needCrop ? REQUEST_FROM_ALBUM :
                    REQUEST_FROM_ALBUM_WITHO_OUT_CROP);
        } else if (obj instanceof Activity) {
            ((Activity) obj).startActivityForResult(albumIntent, needCrop ? REQUEST_FROM_ALBUM :
                    REQUEST_FROM_ALBUM_WITHO_OUT_CROP);
        }
    }

    private static void fromCameraInternal(String planContentId, Object object, boolean needCrop) {
        if (object == null)
            return;
        cropPath = getCropImagePath();
        Intent cameraIntent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
        curTempPhotoPath = AppFileHelper.getPlanCameraPicFileDirById(planContentId) + AppFileHelper
                .createShareImageName();
        Log.i(TAG, "curTempPhotoPath:" + curTempPhotoPath);
        ...
        if (object instanceof Activity) {
            ((Activity) object).startActivityForResult(cameraIntent, needCrop ? REQUEST_FROM_CAMERA :
                    REQUEST_FROM_CAMERA_WITHO_OUT_CROP);
        } else if (object instanceof Fragment) {
            ((Fragment) object).startActivityForResult(cameraIntent, needCrop ? REQUEST_FROM_CAMERA :
                    REQUEST_FROM_CAMERA_WITHO_OUT_CROP);
        }
    }

    private static void toCropInternal(Object obj, Uri imageUri) {
        if (obj == null || imageUri == null)
            return;
    ...
    public static void handleActivityResult(Activity act, int requestCode, int resultCode, Intent data, @Nullable
    ...
    public static void handleActivityResult(Fragment fragment, int requestCode, int resultCode, Intent data,
                                            @Nullable PhotoCallback callback) {
        boolean needCrop = true;
        switch (requestCode) {
            case REQUEST_FROM_CAMERA_WITHO_OUT_CROP:
                needCrop = false;
